...
test("an unsellable request slice keeps retrying to USDC until the timeout, then goes in kind only into accounts the owner already has", async () => {
  ...
  assert.equal(timedOut.requests.paidUsdc, 1, "the converted USDC is paid at the timeout anyway");
  assert.equal(vm.balance(ata(s.alice.publicKey, s.usdc)) - usdcBefore, carved.usdcOwed, "carved USDC paid without waiting for a claim");
  assert.ok(vm.info(r.address), "the undeliverable legs stay on the request, claimable by its owner");
  assert.equal(r.read().usdcOwed, 0n);
  assert.deepEqual(r.read().legAmounts, carved.legAmounts);
  vm.advance(5);
  const again = await tick();
  assert.equal(again.requests.paidUsdc, 0, "nothing more to pay: no repeated empty claims");
  ownerAtas(vm, s, s.alice);
  vm.advance(5);
  ...
  assert.equal(vm.balance(ata(s.alice.publicKey, s.usdc)) - usdcBefore, carved.usdcOwed, "USDC paid exactly once");
  const after = navOf(vm, s);
  assert.equal(after.v.reservedUsdc, 0n);
  assert.ok(after.v.legs.every(l => l.reserved === 0n));
});
