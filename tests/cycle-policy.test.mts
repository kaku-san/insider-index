...
function policy(r = record()): CyclePolicy { return { schema: "insiderindex-cycle-policy-v1", indexId: r.indexId, vault: r.vaultAddress!, shareMint: r.shareMint!, definitionHash: cycleDefinitionHash(r), owner, keeper, operationId: "00000000-0000-4000-8000-000000000001", expiresAt: Date.now() + 60000, approvalReference: null, financialExecutionAuthorized: false,
  economics: { keeperSurplus: "unapproved", shareQuantization: "unapproved", residualCash: "unapproved", issuerAuthorityRiskApproved: false },
  limits: { depositUsdcRaw: "100000000", minNetSharesRaw: "1", minExitUsdcRaw: "99000000", maxOwnerSolDebitLamports: "10000000", maxKeeperSolDebitLamports: "10000000", maxBountyRaw: "0", maxRoundingLossUsdcRaw: "0", maxKeeperSurplusUsdcRaw: "0", swapSlippageBps: 50, rebalanceSlippageBps: 100, perTradeSlippageBps: 50, maxComputeUnits: 1400000, maxMicroLamports: "0", quoteMaxAgeMs: 30000 } }; }
...
test("pilot authority is explicit, expiring and isolated by definition, vault, mint, wallet, keeper and operation", () => {
  ...
  assert.equal(cycleActivationBlockers(p).length, 5);
