...
export async function publicCycleFixture(amountRaw = "100000000", selected?: { templateAmountRaw: string; templateOwner: string }) {
  let policy = cycleTestPolicy();
  const vm = allSevenVm({ owner: policy.owner, keeper: policy.keeper });
  ...
  const access = async () => { await client.discover(selected ? amountRaw : undefined); return client.authorize(accessSignature); };
