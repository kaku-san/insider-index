...
import { kakuSanBuilders, kakuSanConnection, submitKakuSanStep } from "../src/lib/index-vaults/kaku-san-create.ts";
import { observeKakuSanVault, parseKakuSanKeeperArgs, prepareKakuSanKeeperStep } from "../src/lib/index-vaults/kaku-san-rebalance.ts";

const HELP = `Usage:
  npm run keeper:kaku-san -- --dry-run --vault <addr> --share-mint <addr>
  npm run keeper:kaku-san -- --execute --keypath <file> --vault <addr> --share-mint <addr>

Mainnet Kaku San keeper tick. --dry-run is the default mode. --execute requires a keypath
that is not in the web app tree; the Next.js server never loads this key. No --force-rebalance.
JSON on stdout.`;

function assertNotWebAppKeypath(path: string): string {
  const resolved = resolve(path);
  const root = resolve(".");
  for (const dir of ["src", "app", "public", ".next"]) {
    const blocked = resolve(root, dir);
    if (resolved === blocked || resolved.startsWith(`${blocked}/`)) throw new Error("Keeper keypath must not live in the web app tree");
  }
  return resolved;
}

function loadKeeperKey(path: string): Keypair {
  const parsed = JSON.parse(readFileSync(assertNotWebAppKeypath(path), "utf8"));
  ...
  parsed.fill(0);
  if (keypair.publicKey.toBase58() !== KAKU_SAN_DEPLOYER) throw new Error("Keypair is not the approved deployer");
  return keypair;
}
  ...
  try {
    ...
    if (options.mode === "dry-run") {
      console.log(JSON.stringify({
        ...
        status, signerAuthorization: false, transactions: [], broadcasts: 0, simulation: "NOT_RUN",
      }, null, 2));
      return;
    }
    const keypair = loadKeeperKey(options.keypath!);
    ...
    const submit = async (step: "prices" | "rebalance", prepared: Awaited<ReturnType<typeof prepareKakuSanKeeperStep>>) => {
      ...
      const result = await submitKakuSanStep({
        creator: KAKU_SAN_DEPLOYER, step, vault: options.vault, shareMint: options.shareMint, signedTransactions: signed,
      }, kakuSanConnection(true));
      signatures.push(...result.signatures);
    };
    if (status.keeper.intents.length) {
      await submit("prices", await prepareKakuSanKeeperStep({ creator: KAKU_SAN_DEPLOYER, step: "prices", vault: options.vault, shareMint: options.shareMint }, kakuSanBuilders(false)));
    } else if (status.eligibility.required === true) {
      const rebalance = await prepareKakuSanKeeperStep({ creator: KAKU_SAN_DEPLOYER, step: "rebalance", vault: options.vault, shareMint: options.shareMint }, kakuSanBuilders(false));
      if (rebalance.eligible !== true) throw new Error(rebalance.reason ?? "Not eligible to rebalance");
      await submit("rebalance", rebalance);
      await submit("prices", await prepareKakuSanKeeperStep({ creator: KAKU_SAN_DEPLOYER, step: "prices", vault: options.vault, shareMint: options.shareMint }, kakuSanBuilders(false)));
    } else {
      console.log(JSON.stringify({
        schema: "stocklana-kaku-san-keeper-tick-v1", mode: "execute", network: "mainnet-beta",
