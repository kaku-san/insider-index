 ...
 *   npm run nav-vault -- keeper (--all | --indexes a,b | --index <id>) [--network mainnet-beta] [--execute --keypair <keeper file>] [--loop <cycle seconds>] [--marks-every 20] [--rpc-interval-ms 125]
...
const marksConnection = pacedConnection();
...
async function sendConfirmed(keypair: Keypair, via: Connection = connection) {
  return async (tx: VersionedTransaction, step: string) => {
    ...
    const signature = await via.sendRawTransaction(raw, { maxRetries: 0 });
    const deadline = Date.now() + 75_000;
    for (;;) {
      await new Promise(r => setTimeout(r, 2_000));
      const status = (await via.getSignatureStatuses([signature])).value[0];
      ...
      await via.sendRawTransaction(raw, { skipPreflight: true, maxRetries: 0 }).catch(() => undefined);
    }
  };
}
...
async function keeper() {
  ...
  // With --loop, marks post on their own schedule (default every 20 s, well inside the 60 s mainnet
  // max_price_age) so trading work can never make them stale. 0 = marks inside the trading cycle.
  const marksEvery = loop ? Number(opt("--marks-every") ?? 20) : 0;
  ...
  const marksLookupTables = new Map<string, AddressLookupTableAccount>();
  // Leg metadata (pools) changes rarely: read the definitions once per 10 minutes, not every cycle.
  let legCache: { at: number; key: string; legs: PersistedVaultDefinition["vaultLegs"] } | null = null;
  const vaultLegs = async (indexIds: string[]) => {
    const key = indexIds.join(",");
    if (legCache && legCache.key === key && Date.now() - legCache.at < 600_000) return legCache.legs;
    const legs = network === "mainnet-beta" ? (await Promise.all(indexIds.map(id => loadDefinition(id).catch(() => null)))).flatMap(definition => definition?.vaultLegs ?? []) : [];
    legCache = { at: Date.now(), key, legs };
    return legs;
  };
  const venueFor = async (via: Connection, indexIds: string[], states: ReturnType<typeof decodeVault>[], keeperKey: PublicKey) => network === "mainnet-beta"
    ? mainnetVenue({ connection: via, legs: await vaultLegs(indexIds), quoteOwner: keeperKey.toBase58(), env: { JUPITER_API_KEY: process.env.JUPITER_API_KEY } })
    : mockVenue(via, states[0]!.usdcMint);
  const once = async () => {
    rateLimited = 0;
    ...
    const venue = await venueFor(connection, indexIds, states, keeperKey);
    const execute_ = keypair ? await sendConfirmed(keypair) : undefined;
    if (indexIds.length === 1 && !marksEvery) {
    ...
    const cycle = await keeperCycleAll({ connection, indexIds, keeper: keeperKey, programId, ...venue, priorityMicroLamports, ...minTrade, execute: execute_, afterPrices, deferrals, lookupTables, marksPosted: marksEvery > 0 });
  ...
  const marksOnce = async () => {
    const vaults = await keeperVaults(programId, marksConnection);
    if (!vaults.length) return;
    const indexIds = vaults.map(v => v.indexId), states = vaults.map(v => v.state);
    const keeperKey = keypair!.publicKey;
    const venue = await venueFor(marksConnection, indexIds, states, keeperKey);
    const posted = await postMarksAll({ connection: marksConnection, indexIds, keeper: keeperKey, programId, marks: venue.marks, priorityMicroLamports, execute: await sendConfirmed(keypair!, marksConnection), lookupTables: marksLookupTables });
    for (const vault of posted.vaults) if (!vault.ok) log(`marks ${vault.indexId} skipped: ${vault.error?.split("\n")[0]}`);
  };
  if (!loop) return once();
  const every = async (periodSeconds: number, label: string, work: () => Promise<void>) => {
    for (;;) {
      const started = Date.now();
      try { await work(); } catch (error) { log(`${label} failed: ${(error as Error).message}`); }
      // The period is a target, so the cadence holds as the vault count grows.
      await new Promise(r => setTimeout(r, Math.max(1_000, periodSeconds * 1000 - (Date.now() - started))));
    }
  };
  await Promise.all([every(loop, "tick", once), ...(marksEvery ? [every(marksEvery, "marks", marksOnce)] : [])]);
}
