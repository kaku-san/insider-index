...
- Deposits and USDC exits refuse stale marks, and the prepare path also refuses marks within 15 s of `max_price_age` so a signed transaction does not land stale. Deposits also refuse same-slot marks and the keeper as depositor. The client waits for a new mark and re-prepares; it never treats stale preparation as a completed investment. Prepared user transactions carry a compute-unit price (recent vault fees, clamped) so they land on a busy mainnet.
- If free USDC is insufficient, the public prepare path creates a reserved withdrawal request. Keeper conversion may take time. A paused vault, or an owner who explicitly asks for it, can prepare an in-kind exit without a fresh valuation; stale marks alone refuse (retryable) rather than switching an exit to in kind. A healthy but illiquid request becomes owner-claimable in kind after its timeout; keeper/admin may deliver it earlier, only to the owner.
- In-kind claims are chunked at **13 legs per transaction**; owner token accounts that do not exist yet are created inline only when few are missing, otherwise in preceding account-setup transactions, so a 10-11 leg claim stays inside the 64-entry instruction trace limit. A full exit also closes the emptied share account (rent back to the owner). Starting an exit requires one user approval; a later or large in-kind recovery can require additional claims. There is no promise that every exit ends in USDC or completes in the initial transaction.
