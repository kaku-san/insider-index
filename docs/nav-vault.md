...
- **Public rail:** NAV is on by default for every index on mainnet. An index is Live when its NAV vault exists on chain (`GET /api/nav-vault?ids=…`), and otherwise stays Research. The public Symmetry prepare, position and positions routes now serve NAV, and the public Symmetry cycle and devnet routes return 410. Kill switches: `STOCKLANA_NAV_VAULT_DISABLED=1` / `NEXT_PUBLIC_NAV_VAULT_DISABLED=1`. Optional allowlists: `STOCKLANA_NAV_VAULT_INDEXES` / `NEXT_PUBLIC_NAV_VAULT_INDEXES`. The RPC defaults to the server Helius URL.
...
- UI: `TradableSliceNote` / `navSliceLabel` show "Tradable slice: N of M holdings (X% of disclosed weight)" (index, theme and profile pages, and VaultFlow). The Allocation tab keeps every excluded name listed with an asterisk and a plain reason ("No tradable liquidity on Solana yet", "Market too thin right now"), shows the vault target for held names next to the disclosed weight, and ends with "*Not held in the vault yet. Added as soon as trading volume and liquidity improve." Copy lives in `src/lib/frontend/slice-notes.ts`. Readiness reads the slice from the published `insiderindex_nav_vault_slices` table (service role, 5 min memo), falling back to the committed JSON; the disclosure appears whenever that slice matches the on-chain legs, including while the vault is paused or deposits are disabled, without changing Live or Invest gating.
