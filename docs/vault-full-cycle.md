...
2. **Zap in** — USDC → mapped xStocks at target bps. Quotes from Raydium pools, Jupiter if no Raydium pool. Never Pyth/Hermes. Preview is estimated, not guaranteed. Unused USDC is returned.
...
5. **Zap out** — USDC only. The user must not receive a bag of xStocks. Host 0 on exit.
