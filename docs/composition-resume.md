...
- **Exit:** `completeKeepTokens` retains every allocated mint, so WSOL cannot disappear from claim attribution. Public cash-out now uses that all-keep owner claim, then Jupiter sells (`docs/zap-out.md`). It does not use an empty keep mask. The documented dust attempt remains an older empty-keep preflight, not this path. See the [Mag7 cash-out attempt](mag7-usdc-exit-attempt.md) and the [native vault owner contract](../src/lib/index-vaults/README.md).
- **Release:** `VAULT_RELEASE.publicFundsEnabled` and `publicInvestSign` are both true; `nativeUsdcExitVerified` remains false. DB coverage eligibility and the native `depositsAreAllowed` bit were already true for this vault; neither alone means the application accepts funds. The deposit and narrow Mag7 cash-out routes still enforce their own persisted-definition, identity, simulation and UI eligibility gates. External native-program access is not governed by the application's release flag.
