...
- **Exit:** `completeKeepTokens` retains every allocated mint, so WSOL cannot disappear from claim attribution. The current native withdrawal builder preserves in-kind claims; **that is not a USDC-only execution proof**. The zap-out planner requires a WSOL→USDC quote for residual SOL and projects only USDC to the user. Missing quotes fail; estimates are not executable conversion guarantees. Actual native redemption, exact-credit attribution and conversion/claim recovery remain unproved and disabled.
- **Release:** `VAULT_RELEASE.publicFundsEnabled`, `publicInvestSign` and `nativeUsdcExitVerified` remain false. DB coverage eligibility and the native `depositsAreAllowed` bit were already true for this vault; neither means the application accepts funds. This patch does not change either. External native-program access is not governed by the application's release flag.
