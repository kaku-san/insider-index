...
5. Configure browser-visible `NEXT_PUBLIC_SOLANA_RPC_URL` for independent read-only checks. It must support finalized transaction history, address signatures, block signatures, account discovery and simulation. Never expose a private server credential. The wallet trusts this RPC's completeness; pruned, missing, racing or over-limit history refuses signing. Both direct Raydium metadata endpoints must be reachable. No Pyth/Hermes configuration.
