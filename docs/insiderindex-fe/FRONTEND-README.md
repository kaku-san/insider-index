...
`/positions` cards and `/positions/[indexId]` lead with USDC value. The share count is raw mint units (20 raw at 6 decimals stays 20, labeled as raw share units), never the decimal-shifted amount such as 0.00002. The on-chain mint stays 6 decimals. `/positions/[indexId]` is the authoritative native share-position detail and contains the exit entry point, outstanding claims and resumable operations. It shows filled legs versus the published target, not the target mix as holdings. Before Invest or cash-out sign, the sheet shows “Checking every name can be bought…” or “Checking cash out…”, times out to a human line, and enables the action only after that check passes. An unroutable Mag7 amount stays “This amount cannot buy Mag7 right now.” The sheet shows the names actually bought and does not claim the full basket when a name failed. Before signing, cash out says leftover stocks and USDC may be sent to the wallet; that is not a share refund. After completion, it says “Cash out complete — check your wallet.” Do not present a pre-clear holdings snapshot as a post-redemption delivery receipt.
