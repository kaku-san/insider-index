...
Apply `supabase/migrations/202609150001_copy_positions.sql`, then `supabase/migrations/202609150002_prune_copy_orders.sql`, to the production Supabase project before deploying. They create the service-role-only, RLS-enabled `copy_orders` and `positions` tables plus the expired-order pruning RPC. Do not expose the service role to the browser.
...
- Storage is checked before quoting/submission. That readiness boundary prunes expired, unfilled quote contexts after a five-minute grace period and preserves contexts referenced by receipts. Production requires Supabase URL + service role, and never silently falls back to memory. Configured storage failures also fail closed in development.
...
- `/api/positions/copies?wallet=<public key>` is no-store and scoped to that public address. Service-role storage returns at most the latest 100 receipts; the development-only process-memory fallback returns receipts from that process. This is public-address trade history, not authenticated private account data. Database tables themselves are not browser-readable.
