...
export const JUPITER_API_KEY_REQUIRED = "Jupiter API key is required for index buys.";
...
export type JupiterBuildInstruction = { programId: string; accounts: JupiterAccountMeta[]; data: string };
...
export function parseJupiterBuild(body: unknown, expected: { inputMint: string; outputMint: string; amountRaw: string; taker: string }): ParsedJupiterBuild {
  ...
  if (typeof slippageBps !== "number" || BigInt(minOutRaw) > BigInt(outAmount) || !Number.isInteger(slippageBps) || slippageBps < 0 || slippageBps >= 10_000) throw new Error(JUPITER_BUILD_NOT_RAW);
...
export function jupiterBuildUrl(input: { inputMint: string; outputMint: string; amountRaw: string; taker: string; slippageBps?: number }): string {
...
export function requireJupiterApiKey(env: NodeJS.ProcessEnv | { JUPITER_API_KEY?: string | undefined } = process.env): string {
...
export function compileJupiterBuild(parsed: ParsedJupiterBuild): { txBase64: string; messageHash: string; recentBlockhash: string; lastValidBlockHeight: number } {
  ...
  if (tx.message.header.numRequiredSignatures !== 1 || tx.message.staticAccountKeys[0]?.toBase58() !== parsed.taker) throw new Error("Jupiter build payer is not the investing wallet.");
