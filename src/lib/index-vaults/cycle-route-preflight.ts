...
export async function preflightCycleConnectionRoutes(connection: Connection, record: PersistedVaultDefinition, policy: CyclePolicy, metadata: PoolMetadata | undefined, repayments: readonly NativeRepaymentBound[]) {
  ...
  for (const leg of record.vaultLegs) {
    ...
    if (!repayment) throw new Error("CYCLE_NATIVE_REPAYMENT_BOUNDS_UNOBSERVED");
    const buy = await buildCycleRoute({ connection, leg, owner: policy.keeper, inputMint: MAINNET_USDC, outputMint: leg.mint, amountInRaw: String(repayment.outAmount), minimumOutRaw: String(repayment.inAmount), slippageBps: policy.limits.swapSlippageBps, maxAgeMs: policy.limits.quoteMaxAgeMs, metadata });
  ...
  if (observed.size !== record.vaultLegs.length || routes.some(r => r.expiresAt <= Date.now()) || minimum < rawAmount(policy.limits.minExitUsdcRaw)) throw new Error("CYCLE_EXIT_PREREQUISITE_NOT_MET");
