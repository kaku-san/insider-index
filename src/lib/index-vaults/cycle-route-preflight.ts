...
export async function preflightCycleConnectionRoutes(connection: Connection, record: PersistedVaultDefinition, policy: CyclePolicy, metadata: PoolMetadata | undefined, repayments?: readonly NativeRepaymentBound[]) {
  ...
  const totalWeight = record.vaultLegs.reduce((sum, leg) => sum + leg.targetWeightBps, 0);
  if (!repayments?.length && (!Number.isSafeInteger(totalWeight) || totalWeight <= 0)) throw new Error("CYCLE_NATIVE_REPAYMENT_BOUNDS_UNOBSERVED");
  for (const leg of record.vaultLegs) {
    const repayment = observed.get(leg.mint);
    const amountInRaw = repayment ? String(repayment.outAmount) : (rawAmount(policy.limits.depositUsdcRaw) * BigInt(leg.targetWeightBps) / BigInt(totalWeight)).toString();
