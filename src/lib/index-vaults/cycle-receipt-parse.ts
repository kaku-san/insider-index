...
/** Shared browser/server parser. Its caller MUST fetch metadata from a finalized RPC read;
 * a server-supplied balance summary or a client's claimed signature is not a credit. */
export function decodeCycleReceipt(transaction: VersionedTransactionResponse, expected: CycleReceiptExpectation) {
  ...
  if (signature.length !== 64 || !ed25519.verify(signature, message.serialize(), pk(expected.payer).toBytes(), { zip215: false })) throw new Error("CYCLE_RECEIPT_SIGNATURE");
