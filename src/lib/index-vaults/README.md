...
- `kaku-san.ts` / `kaku-san-create.ts`: wallet-signed **execution-test** create for the fixed Kaku San 5-stock mainnet basket (equal 2000 bps, Raydium CLMM only). Hidden at `/kaku-admin` (not linked from nav/footer/home). Only deployer `H6pLh8nFv1teY9G6JAVQuZQxeUvGfwYf1aPXHkDPDRph` may sign; the server never loads a keypair. Prepare is unsigned+simulate; submit broadcasts wallet-signed bytes and returns vault + share mint. Does not enable public Invest Sign.
