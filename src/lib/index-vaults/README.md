...
- `devnet-contract.ts` / `devnet-deposit.ts`: fixed existing test-vault identity, exact USDC input, read-only native preview and explicit preparation blockers; `devnet-invest.md` owns the retained preview's devnet-only contract and W0 visibility.
