...
- `kaku-san.ts` / `kaku-san-create.ts` / `kaku-san-rebalance.ts`: wallet-signed **execution-test** create for the fixed Kaku San mainnet basket (equal 2000 bps, Raydium CLMM only), hidden at `/kaku-admin`. After vault+share mint exist the page shows weight drift versus those 2000 bps targets; it does not sign rebalance. Rebalance is an automated keeper with a dedicated hot wallet (`npm run keeper:kaku-san`, `scripts/kaku-san-keeper-tick.mts`): `--dry-run` default, `--execute --keypair PATH` only on the operator machine, never the deployer Phantom and never a key on the server/app. Existing intents win; no force-rebalance, no public Invest Sign, no deposit/USDC-exit from this page. Native token cap is `MAX_SUPPORTED_TOKENS_PER_VAULT` (100); do not treat the 5-name test basket as a product max.
