...
- `full-cycle.ts` / `zap.ts` / `rebalance-eligibility.ts` / `vault-prices.ts` / `native-caps.ts`: full cycle stages, USDC zap in/out, Hermes-free AND-rule eligibility, Raydium-then-Jupiter quotes, 100-token cap. `tests/full-cycle.test.mts`.
