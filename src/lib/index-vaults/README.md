...
- `raydium-oracles.ts`: **Raydium-only pricing policy.** Rejects `oracle_type: "pyth"` (and every non-Raydium type) before `addOrEditTokenTx`; `mint → Raydium pool + kind` bindings with no invented pools; `planRaydiumPriceUpdate` builds native `update_token_prices` from the vault's lookup-table oracle accounts without the SDK's Hermes-backed `updateTokenPricesTx`; `assertNoPythEnvironment` fails closed on any `HERMES_*`/`PYTH_*` variable. `tests/raydium-oracles.test.mts` exercises the real `NativeVaultBuilders.settle("prices")` path with fixture RPC responses, rejects non-devnet network requests, and verifies the emitted native instruction and bound Raydium accounts.
- `kaku-san.ts` / `kaku-san-create.ts` / `kaku-san-rebalance.ts`: wallet-signed **execution-test** create and keeper rebalance for the fixed Kaku San mainnet basket (equal 2000 bps, Raydium CLMM only). Hidden at `/kaku-admin` (not linked from nav/footer/home). Only deployer `H6pLh8nFv1teY9G6JAVQuZQxeUvGfwYf1aPXHkDPDRph` may sign; the server never loads a keypair. After vault+share mint exist, the page shows weight drift versus those 2000 bps targets and signs Raydium `update_prices` then `rebalanceVaultTx` only if keeper/`isRebalanceRequired` eligibility says so. Existing intents win; no force-rebalance, no public Invest Sign, no deposit/USDC-exit from this page. Native token cap is `MAX_SUPPORTED_TOKENS_PER_VAULT` (100); do not treat the 5-name test basket as a product max. Local tick: `npm run keeper:kaku-san` (`scripts/kaku-san-keeper-tick.mts`), `--dry-run` default, `--execute --keypath` only outside the web app.
...
- `workers/stocklana-keeper.ts`: registered-only read-only tick with a persistent exclusive lease and latched config change monitoring; no unfiltered protocol monitor or sender. Native eligibility is a hint until current native state/fixture checks pass. `workers/devnet-keeper-tick.ts` composes it with strategy decisions for the one existing test vault; `npm run keeper:devnet -- --dry-run` never loads a signer or sends. [Command, evidence and limits](../../../docs/devnet-keeper-tick.md).
