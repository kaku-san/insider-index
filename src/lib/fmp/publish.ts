...
import { PINNED_PERSON_ID, TOP_PROFILE_COUNT, type TopProfilesDocument } from "./top-profiles.ts";
...
export async function publishTopProfiles(db: SupabaseClient, document: TopProfilesDocument): Promise<void> {
  ...
  if (!document.profiles.some((profile) => profile.person.id === PINNED_PERSON_ID)) throw new Error("pelosi-required");
