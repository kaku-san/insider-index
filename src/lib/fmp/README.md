...
- `client.ts`: Node-only bounded FMP transport, key-file fallback and injectable raw archive. Default archive is private `.data/fmp`; operator ingestion/search uses `ingest.ts`’s Supabase raw capture instead. `server.ts` protects runtime credentials with `server-only`.
...
Requests time out at 25 seconds, forbid redirects and retry network/429/5xx at most twice. Retry-After above 30 seconds fails instead of hammering. Upstream errors or malformed JSON are not successful empty results. All captured bodies are credential-redacted before hashing; params never contain credentials. Default filesystem archives are unique `0600` files in a `0700` directory, excluded from git/web/image/rsync; Compose’s private volume persists them. Supabase captures preserve exact redacted body bytes, timestamp, params and SHA-256. Storage failure fails ingestion.
