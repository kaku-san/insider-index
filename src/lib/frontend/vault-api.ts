...
export async function getVaultReadiness(indexId: string): Promise<VaultReadiness | null> {
  ...
  try {
    ...
    // Compatibility with the current devnet pilot described in the design handoff.
    try {
      const legacy = await writeApi<any>("/api/vaults/devnet/preview", { indexId, network: "devnet" });
      const identity = legacy?.identity ?? (legacy?.vault && legacy?.shareMint ? {
        network: "devnet",
        vaultAccount: legacy.vault,
        shareMint: legacy.shareMint,
        indexId,
      } : null);
      return {
        indexId,
        ...
        redeemEnabled: false,
        blockers: legacy?.blockers ?? (legacy?.phase === "BLOCKED" ? ["Vault prepare is blocked on this deployment."] : []),
        observedSlot: legacy?.observedSlot,
        shareSupplyRaw: legacy?.shareSupplyRaw ?? legacy?.shareSupply,
      };
    } catch (legacyError) {
      if (legacyError instanceof ApiError && legacyError.status === 404) return null;
      throw legacyError;
    }
  }
}
...
export async function prepareDeposit(indexId: string, input: { owner: string; amountRaw: RawAmount; idempotencyKey: string; walletProof?: string }): Promise<PreparedStep> {
  return writeApi<PreparedStep>(`/api/indexes/${encodeURIComponent(indexId)}/deposit/prepare`, input);
}

export async function prepareWithdrawal(indexId: string, input: { owner: string; shareAmountRaw: RawAmount; requestedExitMode: "in-kind" | "verified-native-usdc"; idempotencyKey: string; walletProof?: string }): Promise<PreparedStep> {
  return writeApi<PreparedStep>(`/api/indexes/${encodeURIComponent(indexId)}/withdraw/prepare`, input);
}

export async function prepareNext(operationId: string, owner: string, walletProof?: string): Promise<PreparedStep> {
  return writeApi<PreparedStep>(`/api/operations/${encodeURIComponent(operationId)}/next`, { owner, walletProof });
}
...
export async function prepareConversion(operationId: string, owner: string, selectedCreditIds: string[]): Promise<PreparedStep> {
  return writeApi<PreparedStep>(`/api/operations/${encodeURIComponent(operationId)}/convert/prepare`, { owner, selectedCreditIds });
}

export async function prepareRecovery(operationId: string, owner: string): Promise<PreparedStep> {
  return writeApi<PreparedStep>(`/api/operations/${encodeURIComponent(operationId)}/recovery/prepare`, { owner });
}
