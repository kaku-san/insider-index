  ...
  return <div className={styles.workspace}>
    ...
    <div className={styles.indexLayout}>
      <section className={styles.targetPanel} aria-label="Tracker-positions index target">
        ...
        <div className={styles.modelNote}><Icon name="info" size={17} /><p><strong>Proportions, not a NAV.</strong> {index.navDisclaimer} Covers {index.coverage.includedTrackerPercentage.toFixed(2)}% of the {profile.holdingsBasis === "copy-trade-full" ? "copy-trade book" : "tracker book"} out of {index.coverage.listedTrackerPercentage.toFixed(2)}% listed across {index.coverage.holdingsListed} names.</p></div>
        ...
        <details className={styles.sourceDetails}><summary>How this target was built</summary>
          <p>Membership and sizing come only from PelosiTracker’s {profile.holdingsBasis === "copy-trade-full" ? `copy-trade portfolio (${index.coverage.holdingsListed} names)` : `top ${index.coverage.holdingsSlice} positions`} ({index.sourceLabel}, scraped {date(index.asOf)}): a name needs a Solana catalog mint (xStock preferred; verified Backpack on-chain .US when there is no xStock) and an observed mainnet Raydium CLMM/CPMM pool quoted in USDC above the thin-pool floor. No DEX lookalikes are invented. The tracker’s position percentages are renormalized over the included names to 10,000 bps by largest remainder. Share counts and dollar totals never enter. Recent trades are shown on the person page for information and are never an input. The FMP annual disclosure is a separate, older model{fmpIndexHash ? "" : " (none published for this person)"}.</p>
