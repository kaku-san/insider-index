...
export function ProfileView({id, initialData}:{id:string; initialData?: PersonPortfolioResponse}){
 ...
 return <div className={styles.page}>
   ...
   <div className={styles.mobileBar}><button className={following?styles.following:""} onClick={()=>ui.toggleDeviceFollow(id)}>{following?"Following":"Follow"}</button>{indexId?<button onClick={()=>setInvestOpen(true)}>{investLabel}</button>:<button onClick={()=>setShareOpen(true)}>Share</button>}</div>
