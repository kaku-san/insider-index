...
export function ProfileView({id, initialData}:{id:string; initialData?: PersonPortfolioResponse}){
 ...
 return <div className={styles.page}>
   ...
   {tab==="allocation"?<section className={styles.tabSection}>
     <IndexAllocation slice={disclosedSlice} basis={researchRows.length ? "Published target weights; unweighted source holdings remain listed" : trackerPerson ? "Weights in the shown source book" : "Published target weights; unweighted source holdings remain listed"} items={holdings.map(item=>({ticker:item.ticker,name:item.name,weightBps:item.weightPct==null?Number.NaN:item.weightPct*10000,mint:item.mint,issuer:item.venue,tokenSymbol:item.tokenSymbol,network:item.network,detail:item.filingCount&&item.filingCount>1?`${item.filingCount} filings`:null}))}/>
   </section>:null}
