...
function trackerActivity(person:TrackerPerson):ActivityView[]{return person.trades.map((item,index)=>({id:`tracker-${person.id}-${index}`,ticker:item.ticker??"—",name:item.ticker??"Tracked trade",side:eventSide(item.type),tradeDate:item.date,filedDate:item.notificationDate??item.date,amount:typeof item.amount==="number"&&item.amount>0?formatUsd(item.amount):"Range unavailable"}))}
...
export function ProfileView({id, initialData}:{id:string; initialData?: PersonPortfolioResponse}){
 ...
 const holdings=trackerBook.length?trackerBook:(researchBook?.publishedIndex?.constituents?.length?researchHoldings(researchBook):(legacyProfile?.portfolio?.length?legacyHoldings(legacyProfile):researchBook?researchHoldings(researchBook):[]));
 ...
 return <div className={styles.page}>
   ...
   {tab==="holdings"?<section className={styles.tabSection}><div className={styles.tabHeading}><div><h2>Holdings</h2><p>{trackerPerson?"Shown current book as of 2026-09-15 from PelosiTracker. This is not live brokerage NAV.":"Disclosed names stay visible even when there is no approved on-chain mapping."}</p></div>{indexHref?<Link href={indexHref}>Open model index <Icon name="arrow" size={13}/></Link>:null}</div>{holdings.length?<div className={styles.holdingsGrid}>{holdings.map((item,index)=><article className={styles.holdingCard} key={item.key}><span className={styles.rank}>{String(index+1).padStart(2,"0")}</span><StockIcon ticker={item.ticker}/><div className={styles.holdingName}><strong>{item.ticker}</strong><span>{item.name}</span></div><div className={styles.holdingWeight}><strong>{fmtWeight(item.weightPct)}</strong><span>{item.venue??"Disclosure only"}</span></div><div className={styles.weightTrack}><i style={{width:item.weightPct!=null?`${Math.max(3,item.weightPct*100)}%`:"0%"}}/></div></article>)}</div>:<div className={styles.emptyBlock}><strong>No holdings are published.</strong></div>}</section>:null}

   {tab==="activity"?<section className={styles.tabSection}><div className={styles.tabHeading}><div><h2>Public moves</h2><p>{trackerPerson?"Information only. Tracker trades never rewrite the current book and are not copyable from this list.":"Copy appears only when the disclosure is eligible and has a real disclosure ID."}</p></div><Link href="/feed">Open Feed</Link></div>{!activity.length?<div className={styles.emptyBlock}><strong>No activity is saved.</strong></div>:<div className={styles.movesList}>{activity.map((item,index)=><article className={styles.moveRow} key={item.id}><span className={styles.moveIndex}>{String(index+1).padStart(2,"0")}</span><StockIcon ticker={item.ticker} size="sm"/><span className={`${styles.moveSide} ${styles[item.side]}`}>{item.side==="buy"?"BOUGHT":item.side==="sell"?"SOLD":"FILED"}</span><div className={styles.moveAsset}><strong>{item.ticker}</strong><span>{item.name}</span></div><div className={styles.moveDates}><span><b>Trade</b>{shortDate(item.tradeDate)}</span><span><b>Filed</b>{shortDate(item.filedDate)}</span></div><div className={styles.moveAmount}><strong>{item.amount}</strong>{item.copyHref?<Link href={item.copyHref}>Copy this print</Link>:item.sourceUrl?<FilingLink url={item.sourceUrl}/>:<span>Research only</span>}</div></article>)}</div>}</section>:null}
