...
export function FeedView({initialDisclosures}:{initialDisclosures?:Disclosure[]}={}){const ui=useUI(),wallet=usePrivySolana(),params=useSearchParams();
 const disclosuresRes=useResource<{disclosures:Disclosure[]}>("/api/disclosures",initialDisclosures?{disclosures:initialDisclosures}:undefined),followsRes=useResource<{follows:Follow[]}>(wallet.solanaAddress?`/api/follows?wallet=${encodeURIComponent(wallet.solanaAddress)}`:null);
 const [side,setSide]=useState("all"),[savedOnly,setSavedOnly]=useState(false),[sort,setSort]=useState("latest");
 const disclosures=disclosuresRes.data?.disclosures??[],follows=followsRes.data?.follows??[];
 ...
 const visible=signals.filter(s=>inLane(s)&&`${s.insiderName} ${s.ticker} ${s.issuerName} ${s.xstockSymbol??""}`.toLowerCase().includes(needle)&&(side==="all"||s.side===side)&&(!savedOnly||ui.saved.includes(s.id))).sort((a,b)=>sort==="latest"?Date.parse(b.filedAt)-Date.parse(a.filedAt):Date.parse(a.filedAt)-Date.parse(b.filedAt));
 ...
 const queueIds=new Set(follows.filter(f=>f.autoCopy).map(f=>f.profileId)),queue=signals.filter(s=>queueIds.has(s.profileId)&&s.side==="buy"&&s.tradeEligible);
 ...
 {queue.length>0&&ui.lane==="following"?<div className="notice"><Icon name="bell" size={17}/>{queue.length} buy{queue.length===1?"":"s"} from people you follow are ready to copy. Each one needs your signature.</div>:null}
 <div className="signal-list">{visible.map(s=><SignalCard signal={s} key={s.id}/>)}</div>
 {visible.length===0?<EmptyState title={ui.lane==="following"&&!followed.size?"You’re not following anyone yet.":"No filings match. Yet."} description={ui.lane==="following"&&!followed.size?"Open a profile and hit Follow. Their public filings will land right here, and their next allowlisted buy is one tap to copy.":"Try another ticker or filter. We won’t invent a filing."} action={ui.lane==="following"&&!followed.size?"See who’s filing":"Clear filters"} onAction={()=>{clearFilters();if(ui.lane==="following"&&!followed.size)chooseLane("live");}}/>:null}
