...
export function IndexCard({index,readiness,needsRebalance=false}: {index:PersonIndex;readiness?:IndexReadiness;needsRebalance?:boolean}) {
 ...
 <p>{crowd?"The xStocks these filers bought most, weighted by how many of them bought.":"Their disclosed buys, as one basket of allowlisted xStocks."}</p>
 <div className="weight-bar" aria-hidden="true">{index.constituents.map((h,i)=><span key={h.mint} style={{width:`${h.weightPct*100}%`,background:CHART_COLORS[i%CHART_COLORS.length]}}/>)}</div>
 <div className="index-weight-chips">{index.constituents.slice(0,6).map((h,i)=><span key={h.mint}><i style={{background:CHART_COLORS[i%CHART_COLORS.length]}}/>{h.ticker}<strong>{(h.weightPct*100).toFixed(0)}%</strong></span>)}{index.constituents.length>6?<span>+{index.constituents.length-6} more</span>:null}</div>
