  ...
  async function approve() {
    ...
    try {
      ...
      for (const transaction of prepared.transactions) {
        ...
        receipts.push({ stepId: transaction.stepId, signature });
      }
      setOperation(await submitReceipts(prepared.operationId, wallet.solanaAddress, receipts));
      setPrepared(null);
    } catch (caught) {
      setError(errorText(caught));
    } finally {
