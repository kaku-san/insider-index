...
import {
  depositIsEnabled, getIndexPosition, navSliceLabel, hasIndexShares, positionValueUsdc, prepareDeposit, prepareWithdrawal,
...
import { SLICE_FOOTNOTE } from "@/lib/frontend/slice-notes";
...
export function VaultFlow({ open, onClose, indexId, indexName, readiness, mode = "deposit", position, onPosition }: { open: boolean; onClose: () => void; indexId: string; indexName: string; readiness?: VaultReadiness | null; mode?: Mode; position?: IndexSharePosition | null; indexKind?: "person" | "theme"; onPosition?: (position: IndexSharePosition | null) => void }) {
  ...
  const [amount, setAmount] = useState(() => mode === "deposit" ? "10" : positionSharesText(position) ?? "0");
  ...
  // NAV vault (flagged): USDC in + shares out in one approval; exits pay USDC or the pro-rata basket.
  const isNav = readiness?.kind === "nav-vault";
  const usdcMint = readiness?.identity?.usdcMint ?? (network === "devnet" ? "USDCoctVLVnvTXBEuP9s8hntucdJokbo17RwHuNXemT" : "EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v");
  ...
  useEffect(() => {
    ...
    let alive = true;
    const endpoint = network === "devnet" ? "https://api.devnet.solana.com" : `${window.location.origin}/api/rpc`;
    void fetch(endpoint, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ jsonrpc: "2.0", id: "usdc-balance", method: "getTokenAccountsByOwner", params: [wallet.solanaAddress, { mint: usdcMint }, { encoding: "jsonParsed", commitment: "confirmed" }] }) })
  ...
  useEffect(() => {
    ...
    const startedAt = Date.now(), deadline = startedAt + SETTLEMENT_POLL_TIMEOUT_MS;
    // Re-read quickly right after the signature confirms, then fall back to the settlement cadence.
    const nextPollMs = () => Date.now() - startedAt < POSITION_REFRESH_WINDOW_MS ? POSITION_REFRESH_POLL_MS : SETTLEMENT_POLL_MS;
    const observe = () => void getIndexPosition(indexId, wallet.solanaAddress!).then(value => {
      ...
      if (next.listening) timer = setTimeout(observe, nextPollMs());
    }).catch(() => {
      if (!alive) return;
      if (mode === "deposit" && Date.now() >= deadline) setTimedOut(true);
      else timer = setTimeout(observe, nextPollMs());
    });
    observe();
  ...
  return <><div className={styles.backdrop} onMouseDown={event => { if (event.currentTarget === event.target) onClose(); }}><aside className={styles.sheet} role="dialog" aria-modal="true" aria-label={title}>
    ...
    <div className={styles.body}>
      ...
      {displayed === "amount" ? <><div className={styles.intro}><h3>{mode === "deposit" ? "Invest in USDC." : "Cash out."}</h3><p>{isNav ? (mode === "deposit" ? `One approval: your USDC goes into the vault and your shares mint in the same transaction at the current vault value${readiness?.hostEntryFeeBps ? ` (${(readiness.hostEntryFeeBps / 100).toFixed(2)}% entry fee)` : ""}. The keeper buys the index names afterwards, so part of your share is cash pending investment.` : "One approval: your shares burn and you receive USDC from the vault's cash buffer. If the buffer is short, you receive your exact share of the vault's stocks plus USDC instead.") : mode === "deposit" ? "Your USDC buys each name at its weight. Those tokens become index shares. Unspent USDC stays in your wallet. A name that does not buy is not counted." : CASH_OUT_BEFORE_SIGN}</p></div>
        ...
        <div className={styles.summary}><div className={styles.row}><span>{mode === "deposit" ? "You invest" : "You cash out"}</span><strong>{mode === "deposit" ? `${amount || "0"} USDC` : `${amount} shares`}</strong></div>{mode === "deposit" && wallet.solanaAddress ? <div className={styles.row}><span>Investing as</span><strong>{wallet.solanaAddress.slice(0, 8)}…{wallet.solanaAddress.slice(-8)}</strong></div> : null}{mode === "deposit" ? <div className={styles.row}><span>USDC in this wallet</span><strong>{wallet.solanaAddress ? <>{availableUsdcText()} USDC <button type="button" className={styles.max} disabled={availableUsdcRaw === null} onClick={useMaxUsdc}>Max</button></> : "Connect wallet to read"}</strong></div> : null}{mode === "deposit" && holdingsLabel ? <div className={styles.row}><span>Your position</span><strong>{holdingsLabel}</strong></div> : null}{mode === "withdraw" || isNav ? <div className={styles.row}><span>Wallet approvals</span><strong>1 approval now</strong></div> : null}{isNav && navSliceLabel(readiness) ? <div className={styles.row}><span>Holds</span><strong>{navSliceLabel(readiness)}</strong></div> : null}{isNav && navSliceLabel(readiness) && readiness?.slice?.excluded.length ? <div className={styles.row} data-slice-excluded="true"><span>Not held yet*</span><strong>{readiness.slice.excluded.map(item => `${item.ticker}*`).join(", ")}</strong></div> : null}</div>{isNav && navSliceLabel(readiness) && readiness?.slice?.excluded.length ? <p className={styles.sliceFootnote} data-slice-footnote="true">{SLICE_FOOTNOTE}</p> : null}
        ...
        : displayed === "approval" ? <><div className={styles.intro}><h3>Approve in your wallet.</h3><p>{isNav ? `Check the amount before you sign. ${prepared?.step.estimate?.outputSummary ?? ""}` : mode === "withdraw" ? `Check the amount before you sign. ${CASH_OUT_BEFORE_SIGN}` : prepared?.step.basket?.statusLine ?? "Check the amount before you sign. Shares are not claimed until every quoted name is bought."}</p></div>{busy ? <p role="status">Waiting for wallet…</p> : null}{error ? <div className={styles.blockers}>{error}</div> : null}<div className={styles.cta}><button className={styles.secondary} onClick={() => setScreen("amount")}>Back</button><button className={styles.primary} disabled={busy || !prepared?.step.transactions.length} onClick={() => void approve()}>{busy ? "Waiting for wallet…" : "Approve in wallet"}</button></div></>
    ...
    </div><footer className={styles.footer}>Target mix and share balances are separate from a completed buy. Fees and share amounts are shown only when preparation supplies them.</footer>
