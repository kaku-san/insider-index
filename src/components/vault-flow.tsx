...
export function VaultFlow({open,onClose,indexId,indexName,readiness,mode="deposit",position}:{open:boolean;onClose:()=>void;indexId:string;indexName:string;readiness?:VaultReadiness|null;mode?:Mode;position?:IndexSharePosition|null;indexKind?:IndexKind}){
  ...
  const [amount,setAmount]=useState(()=>mode==="deposit"?PUBLIC_DEPOSIT_MINIMUM_USDC:positionSharesText(position)??"0");
  ...
  useEffect(()=>{if(open){setScreen("amount");setPrepared(null);setOperation(null);setSettlementPosition(null);setError(null);setAmount(mode==="deposit"?PUBLIC_DEPOSIT_MINIMUM_USDC:positionSharesText(position)??"0");}},[open,mode,indexId,position?.sharesText,position?.sharesRaw,position?.shareDecimals]);
