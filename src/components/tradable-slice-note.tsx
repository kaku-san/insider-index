...
import { plainSliceReason, SLICE_FOOTNOTE } from "@/lib/frontend/slice-notes";
...
export function TradableSliceNote({ readiness, className }: { readiness?: VaultReadiness | null; className?: string }) {
  ...
  return <div className={className} data-tradable-slice="true">
    ...
    {excluded.length ? <details>
      <summary>{excluded.length} disclosed {excluded.length === 1 ? "name is" : "names are"} not held yet*</summary>
      <ul>{excluded.map(item => <li key={`${item.ticker}-${item.reason}`}>{item.ticker}* — {plainSliceReason(item.reason)}</li>)}</ul>
      <p>{SLICE_FOOTNOTE}</p>
    </details> : null}
  </div>;
}
