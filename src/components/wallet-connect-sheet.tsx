...
export function WalletConnectSheet({open,onClose}:{open:boolean;onClose:()=>void}){
  ...
  return <div className={styles.backdrop} onMouseDown={(e)=>{if(e.currentTarget===e.target)onClose()}}>
    <section className={styles.sheet} role="dialog" aria-modal="true" aria-label="Connect to InsiderIndex">
      ...
      {wallet.authenticated&&wallet.solanaAddress?<><div className={styles.connected}><div className={styles.walletCard}><div><small>{wallet.connectionMethod==="email"?"Embedded Solana wallet":"Connected Solana wallet"}</small><code>{shortenAddress(wallet.solanaAddress,8)}</code></div><Icon name="check" size={22}/></div><button className={styles.disconnect} disabled={Boolean(busy)} onClick={()=>void disconnect()}>{busy==="disconnect"?"Disconnecting…":"Disconnect"}</button></div>{wallet.solanaWallets.length>1?<div className={styles.choices}>{wallet.solanaWallets.map(address=><button key={address} className={styles.walletPick} disabled={Boolean(busy)||address===wallet.solanaAddress} onClick={()=>wallet.selectSolanaWallet(address)}><strong>{address===wallet.solanaAddress?"Using this wallet":"Use this wallet"}</strong><span>{shortenAddress(address,8)}</span></button>)}</div>:null}</>:<div className={styles.choices}>
