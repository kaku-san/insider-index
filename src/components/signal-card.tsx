...
export function SignalCard({signal}: {signal:CopySignal}) {
 ...
 return <article className="signal-card"><div className="signal-header"><Link href={`/p/${encodeURIComponent(signal.profileId)}`} className="signal-person"><PersonAvatar name={signal.insiderName} imageUrl={signal.imageUrl}/><div><strong>{signal.insiderName}</strong><div className="signal-person-meta"><PartyBadge party={signal.party} kind={signal.kind}/><span>{mock?"Example filing":formatDate(signal.filedAt)}</span></div></div></Link><button className={`icon-button save-button ${saved?"is-saved":""}`} aria-label={saved?`Unsave ${signal.ticker} filing`:`Save ${signal.ticker} filing`} aria-pressed={saved} onClick={()=>{ui.toggleSave(signal.id);ui.toast(saved?"Filing removed from saved.":"Filing saved on this device.");}}><Icon name="bookmark" size={18}/></button></div>
