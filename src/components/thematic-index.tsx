...
import { IndexPerformancePlaceholder } from "./consumer-index";
...
export function ThematicIndexPage({ id, initialData }: { id: string; initialData?: Payload }) {
  ...
  async function share() {
    ...
    try {
      if (navigator.share) await navigator.share({ title: index?.indexName ?? "InsiderIndex theme", url });
      else {
  ...
  return <div className={styles.page}>
    ...
    <section className={styles.hero}>
      ...
      <div className={styles.heroCopy}>
        ...
        <p>{index.headline}</p>
        <div className={styles.proof}>{holdings.length} holdings · recurring cross-filer evidence · 100% catalog mapped</div>
        <div className={styles.actions}>
          <Link className={styles.primary} href="#holdings">View holdings <Icon name="arrow" size={14} /></Link>
    ...
    <section className={styles.statStrip}>
      ...
      <div><span>Coverage</span><strong>100%</strong><small>catalog mapped</small></div>
      <div><span>Status</span><strong>Research</strong></div>
    ...
    <section className={styles.tabContent}>
      ...
      {tab === "holdings" ? <div className={styles.holdingsTable}>
        ...
        {holdings.map((item) => <div className={styles.fullHolding} key={item.mint}><div><StockIcon ticker={item.ticker} size="md" /><span><strong>{item.ticker}</strong><small>{companyNameFor(item.ticker, item.name)}</small></span></div><b>{(item.weight_bps / 100).toFixed(item.weight_bps >= 1000 ? 1 : 2)}%</b><span>{item.issuer === "xstock" ? "xStock" : "Backpack"}</span><em>Mapped</em></div>)}
      </div> : null}
      ...
      {tab === "about" ? <div className={styles.aboutGrid}>
        ...
        <section className={styles.disclaimer}><span>IMPORTANT</span><h3>Research only</h3><p>{index.disclaimers.join(" ")}</p></section>
      </div> : null}
    </section>
  </div>;
}
