...
export function IndexHome({initialIndexes,initialDisclosures}:{initialIndexes?:PersonIndex[];initialDisclosures?:Disclosure[]}={}){
 ...
 <section className="explainer-grid" aria-label="Index or copy one trade"><div className="panel explainer"><span className="mini-label">OPTION ONE</span><h3><Icon name="grid" size={17}/>Buy an index</h3><p>One basket of the names real filers actually hold, weighted by their disclosed size. xStock legs are quoted and signed in-app; legs that only trade elsewhere are marked by venue. When a new filing changes the weights, you review the rebalance before anything moves.</p><span className="explainer-when">Best when many people are buying the same names.</span></div><div className="panel explainer"><span className="mini-label">OPTION TWO · THE FALLBACK</span><h3><Icon name="copy" size={17}/>Follow &amp; copy one trade</h3><p>Follow a person. When they disclose a buy in a name we can route — an xStock on Solana, or a market on Backpack — you can buy the same name at today’s price, after you approve it. Names we can’t route yet still show in their book, just not as a trade.</p><span className="explainer-when">Best when there aren’t enough filings for a basket yet.</span></div></section>
