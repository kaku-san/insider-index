...
export function IndexHome({initialIndexes,initialDisclosures}:{initialIndexes?:PersonIndex[];initialDisclosures?:Disclosure[]}={}){
 ...
 {ready.length?<div className="index-grid">{ready.map(r=><IndexCard key={r.index.id} index={r.index} readiness={r.readiness}/>)}</div>:<EmptyState title="No index is ready yet." description={`A crowd index needs at least ${INDEX_RULES.crowd.minFilers} filers buying ${INDEX_RULES.crowd.minNames} tradable names in ${INDEX_RULES.windowDays} days; a person index needs ${INDEX_RULES.person.minNames} tradable names in their disclosed book. We won’t pad it. Until then, follow a filer below and copy their next buy.`} action={followable.length?"Follow someone instead":undefined} onAction={followable.length?()=>scrollTo("follow-copy"):undefined}/>}
 {building.length?<div className="building-list">{building.map(r=><div key={r.index.id} className="building-row"><PersonAvatar name={r.index.name} imageUrl={null} size="sm"/><div><strong>{r.index.name}</strong><span>{r.readiness.need} So far: {r.readiness.filers} filer{r.readiness.filers===1?"":"s"}, {r.readiness.names} tradable name{r.readiness.names===1?"":"s"}.</span></div><Link className="text-button" href={`/indexes/${encodeURIComponent(r.index.id)}`}>Watch it fill<Icon name="up" size={13}/></Link></div>)}</div>:null}
 </section>

 <section className="explainer-grid" aria-label="Index or copy one trade"><div className="panel explainer"><span className="mini-label">OPTION ONE</span><h3><Icon name="grid" size={17}/>Buy an index</h3><p>One basket of the names real filers actually hold, weighted by their disclosed size. Every leg is a Solana token you swap into and sign for — the xStock mint where one exists, otherwise the Backpack token — each tagged by issuer. When a new filing changes the weights, you review the rebalance before anything moves.</p><span className="explainer-when">Best when many people are buying the same names.</span></div><div className="panel explainer"><span className="mini-label">OPTION TWO · THE FALLBACK</span><h3><Icon name="copy" size={17}/>Follow &amp; copy one trade</h3><p>Follow a person. When they disclose a buy in a name with a Solana mint — an xStock, or a Backpack tokenised stock — you can buy the same name at today’s price, after you approve it. Names we can’t route yet still show in their book, just not as a trade.</p><span className="explainer-when">Best when there aren’t enough filings for a basket yet.</span></div></section>

 <div className="discovery-layout"><div className="feed-column">
 <section id="follow-copy" className="home-section"><div className="section-heading"><div><h2><Icon name="people" size={19}/>Follow &amp; copy<span className="heading-dot"/></h2><p>People with a recent buy on a name we can route. Follow them, or copy the exact trade.</p></div></div>
 ...
 {followable.length?<div className="filer-list">{followable.slice(0,8).map(f=><div key={f.id} className="filer-row"><Link href={`/p/${encodeURIComponent(f.id)}`} className="filer-identity"><PersonAvatar name={f.name} imageUrl={portraitFor(f.id)}/><div><strong>{f.name}</strong><span>{f.kind==="politician"?"Congress":"Executive"} · {f.filings} filing{f.filings===1?"":"s"} · bought <b>{f.latestBuy!.venueSymbol??f.latestBuy!.ticker}</b> {PREVIEW_MODE?"(example)":formatDate(f.latestBuy!.filedAt)}</span></div></Link><StockIcon ticker={f.latestBuy!.ticker} size="sm"/><div className="filer-actions"><FollowButton profileId={f.id} follow={follows.find(x=>x.profileId===f.id)} onChanged={followsRes.reload} compact/><CopyButton signalId={f.latestBuy!.id} enabled venue={f.latestBuy!.venue} label="Copy this buy"/></div><span className="filer-note">{readyIds.has(f.id)?<Link href={`/indexes/${encodeURIComponent(f.index!.id)}`}>Also has an index<Icon name="up" size={11}/></Link>:<Link href={`/p/${encodeURIComponent(f.id)}`}>See their full disclosed book<Icon name="up" size={11}/></Link>}</span></div>)}</div>:<EmptyState title="Nobody has a recent tradable buy." description={`No filer bought a name we can route in the last ${INDEX_RULES.windowDays} days. Nothing to copy yet — check the feed for everything else.`} action="Open the feed" onAction={()=>router.push("/feed")}/>}
 ...
 <aside className="discovery-rail"><section className="rail-card"><div className="rail-heading"><h3><Icon name="eye" size={18}/>Most bought</h3><span>{INDEX_RULES.windowDays}d</span></div><p className="rail-subtitle">Distinct filers buying each tradable name</p>{radar.length?<div className="radar-list">{radar.map(([ticker,count],i)=><Link key={ticker} href={`/feed?q=${encodeURIComponent(ticker)}`}><span className="radar-rank">{i+1}</span><StockIcon ticker={ticker} size="sm"/><strong>{ticker}</strong><span>{count} filer{count===1?"":"s"}</span><Icon name="up" size={14}/></Link>)}</div>:<p className="small-text muted">No tradable buys in the window.</p>}</section>
