...
- Native indexes: `src/lib/index-vaults/README.md` owns integration/release status; `adapter-contract.ts` is the app boundary. One Symmetry V3 vault/native share mint per index; no receipt wrapper or purchase-row ownership. All vault funding/signing stays disabled pending native evidence and explicit budget authorization. Legacy index quote/execute are retired; individual copy trades remain separate.
