...
- PelosiTracker top-20 handoff: `src/lib/tracker/README.md`. Keep both zips verbatim: slice-only `pelositracker-top20-handoff/` and product dataset `pelositracker-top20full-handoff/` (each with MANIFEST.json + test guard); photos mirror to `public/tracker/photos/`. Copy-trade full holdings where present (Pelosi 15, MTG 76) are the shown current book; everyone else is top 5 + OTHER (label as slice, do not invent tickers). FMP is the older annual disclosure. Trades are info only; nothing is summed and no tracker dollar is a net worth or NAV. Copy-trade $, politician-API $, and vault NAV are three figures — never mixed. The vault-ready tracker-positions index (`tracker-index.ts`, component `src/components/tracker-index.tsx` at `/indexes/tracker-<bioguide>`) weights xStock preferred, verified Backpack `.US` when there is no xStock, plus observed Raydium USDC pool from `raydium-pools-mainnet.json` (`npm run raydium:snapshot`, never hand-edit). Exit is USDC only; nothing enables invest/exit.
