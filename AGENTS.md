...
- Person portfolios: compact consumer page is `src/components/profile-view.tsx`. `src/components/fmp-portfolio.tsx` still renders saved annual rows independently of publication; `src/lib/fmp/index-name.ts` owns canonical index/vault names. Never filter a disclosed book by tradability. Legacy basket readiness is `src/lib/fomo/index-readiness.ts`; crowd indexes are `src/lib/fomo/crowd-indexes.ts`.
- PelosiTracker top-20 handoff: `src/lib/tracker/README.md`. The zip lives verbatim in `data/insiderindex-source-buckets/pelositracker-top20-handoff/` (MANIFEST.json + test guard); photos mirror to `public/tracker/photos/`. Tracker positions (as of 2026-09-15) are the shown current book, FMP is the older annual disclosure, trades are info only; nothing is summed and no tracker dollar is a net worth or NAV. The tracker-positions index (`tracker-index.ts`) weights only mint + observed Raydium USDC pool names from `raydium-pools-mainnet.json` (`npm run raydium:snapshot`, never hand-edit). Exit is USDC only; nothing enables invest/exit.
