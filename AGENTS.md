...
- Native indexes: `src/lib/index-vaults/README.md` owns integration/release status; `adapter-contract.ts` is the app boundary. One Symmetry V3 vault/native share mint per index; no receipt wrapper or purchase-row ownership. Vault signing fails closed pending native readiness. Legacy index quote/execute are retired; individual copy trades remain separate.
