...
- Native indexes: `src/lib/index-vaults/README.md` owns integration/release status; `adapter-contract.ts` is the app boundary. One Symmetry V3 vault/native share mint per index; no receipt wrapper or purchase-row ownership. Vault funding/signing fails closed pending native readiness and explicit budget authorization. Legacy index quote/execute are retired; individual copy trades remain separate. Kaku San execution-test create/rebalance is `/kaku-admin` only (`kaku-san.ts`, `kaku-san-rebalance.ts`); do not link it from nav/home, do not load a keeper keypair on the server, do not force-rebalance when native eligibility is false, and do not enable public Invest Sign. Local tick: `npm run keeper:kaku-san -- --dry-run` (default); `--execute` only with `--keypath` in `scripts/kaku-san-keeper-tick.mts`.
- **Symmetry prices are Raydium-only.** `src/lib/index-vaults/raydium-oracles.ts` rejects `oracle_type: "pyth"`, maps `mint → Raydium pool` (never invent one), and builds `update_token_prices` without the SDK's `updateTokenPricesTx` (it always opens a Hermes client). Never add Hermes/Pyth clients, keys or `HERMES_*`/`PYTH_*` env; `tests/raydium-oracles.test.mts` executes the real settlement price builder against fixtures, rejects non-devnet network requests, and verifies the native Raydium instruction and accounts. Devnet settlement on the one existing test vault: `npm run vault:settle:devnet` (`docs/devnet-vault-settle.md`); receipts in `evidence/vaults/DEVNET_TEST_VAULT.md`.
