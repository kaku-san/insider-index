...
- FMP saved books and trade indexes: `src/lib/fmp/README.md` → Supabase-only `/api/people` and `/api/people/[id]/portfolio`. `trade-index.ts` builds separate activity-weighted targets from trade symbols, even with partial annual pages; never rewrite annual rows or route FMP through the legacy PTR-netted calculator. Publication uses the owner RPC in `supabase/migrations/202609140002_trade_indexes.sql`.
