...
Set server-only `FMP_API_KEY`, or use `$HOME/.config/fmp-api-key` for local development. Requests use the `apikey` **header**, never a browser secret or query credential. Raw observations (including errors) are redacted. Operator ingestion/search persists them only to Supabase via `supabaseArchive`, with fetch time, request parameters, SHA-256 and page metadata; an unconfigured archive skips persistence. No FMP archive writes the app disk or falls back to local files, so read-only Vercel functions are safe. No raw archive is web-served. A configured Supabase archive failure remains an ingestion failure. See `src/lib/fmp/README.md`.
