-- A public depositor may replace an amount selected before any contribution/share progress.
-- This is deliberately narrower than a general policy edit: the operation identity and every
-- other state field remain byte-for-byte identical, and an unresolved draft must be reconciled.
begin;
...
begin
 ...
 if p_state->>'operationId' is distinct from p_operation_id::text or p_state->>'indexId' is distinct from r.index_id
   ...
   or p_state->>'keeper' is distinct from r.state->>'keeper' or p_state->>'definitionHash' is distinct from r.state->>'definitionHash'
 then raise exception 'CYCLE_IDENTITY_CONFLICT'; end if;
 -- A policy hash/approved amount pair can change only for a pre-funding amount restart. No
 -- unsigned wire is discarded here: canonical reconciliation must clear it first.
 if p_state->>'policyHash' is distinct from r.state->>'policyHash'
   or p_state->>'approvedDepositUsdcRaw' is distinct from r.state->>'approvedDepositUsdcRaw' then
   if (r.state - 'policyHash' - 'approvedDepositUsdcRaw') is distinct from (p_state - 'policyHash' - 'approvedDepositUsdcRaw')
