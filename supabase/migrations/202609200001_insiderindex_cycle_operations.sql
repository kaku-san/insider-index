...
begin
 if p_token is null or p_initial->>'operationId' is distinct from p_operation_id::text
   ...
   or p_initial->'receipts' is distinct from '[]'::jsonb or p_initial->'credits' is distinct from '[]'::jsonb
...
begin
 ...
 then raise exception 'CYCLE_IDENTITY_CONFLICT'; end if;
 if jsonb_typeof(p_state->'receipts') is distinct from 'array' or jsonb_typeof(p_state->'credits') is distinct from 'array' then raise exception 'CYCLE_JOURNAL_SHAPE'; end if;
 ...
 if done and (p_state->>'pending' is not null or p_state->>'nativeClaimsClear' is distinct from 'true' or exists(
